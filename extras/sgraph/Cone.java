...
public class Cone implements IShape {
    ...
    public HitRecord intersect(Ray ray, Stack<Matrix4f> modelView) {
        ...
        float B = 2 * (direction.x * start.x - direction.y * (start.y - 1) + direction.z * start.z);
        float C = start.x * start.x - (start.y - 1) * (start.y - 1) + start.z * start.z;
        ...
        if (p.y > 0 && p.y < 1 && tSideMin > 0) {
            ...
            normal = normal.add(new Vector4f(0, 1, 0, 0));
            tMin = tSideMin;
            intersected = true;
        }
        ...
        if (direction.y != 0) {
            ...
            if (tBaseMin > 0) {
                if (Math.abs(p.y - 1) < threshold && Math.abs(p.x) < threshold && Math.abs(p.z) < threshold) {
                    normal = new Vector4f(0, 1, 0, 0);
                    tMin = tBaseMin;
                    intersected = true;
                } else if (Math.abs(p.y) < threshold && (p.x * p.x + p.z * p.z - 1 <= threshold)) {
                    normal = new Vector4f(0, -1, 0, 0);
                    tMin = tBaseMin;
                    intersected = true;
                }
        ...
        if (intersected) {
            ...
            float theta = (float) Math.acos(p.x / 1);
